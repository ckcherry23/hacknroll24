import { TelegramMessage } from "@/lib/types";

export const messages: TelegramMessage[] = [
  {
    message: "Saw the new intern today. He seems hardworking.",
    sender: "Elon",
    imageUrl: "/elonmusk.jpg",
    delay: 0
  },
  {
    message: "Yes sir, he's been veeery hardworking.",
    sender: "Dave",
    imageUrl: "https://avatars.githubusercontent.com/u/3?v=5",
    delay: 1
  },
  {
    message: "Good to see we have willing talent in the company. Hell, maybe he'll even take over your role, Dave. Hahaha",
    sender: "Elon",
    imageUrl: "/elonmusk.jpg",
    delay: 2
  },
  {
    message: "Yeah right. As if that nincompoop can achieve anything great",
    sender: "John",
    imageUrl: "https://avatars.githubusercontent.com/u/1?v=5",
    delay: 3
  },
  {
    message: "Yeah, that guy's just an intern. What'd the company do if we lost me, the person in charge of messaging, vs the noodle-headed intern?",
    sender: "Dave",
    imageUrl: "https://avatars.githubusercontent.com/u/?v=5",
    delay: 4
  },
]